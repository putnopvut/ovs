...
/* Remove entries in extend table, such as group/meter table, created
 * by the lflow with sb_uuid */
static void
remove_ext_info_for_lflow(const struct uuid *sb_uuid,
                                struct ovn_extend_table *ext)
{
    struct ovn_extend_table_info *e, *next_e;
    HMAP_FOR_EACH_SAFE (e, next_e, hmap_node, &ext->desired) {
        if (uuid_equals(&e->lflow_uuid, sb_uuid)) {
            hmap_remove(&ext->desired, &e->hmap_node);
            ds_destroy(&e->info);
            free(e);
        }
    }
...
ofctrl_remove_flows(const struct uuid *sb_uuid)
{
    ...
    remove_ext_info_for_lflow(sb_uuid, groups);
    remove_ext_info_for_lflow(sb_uuid, meters);
}
           ...
           bool flow_changed)
{
    ...
    HMAP_FOR_EACH (d, match_hmap_node, &match_flow_table) {
        i = ovn_flow_lookup(&installed_flows, d);
        if (!i) {
