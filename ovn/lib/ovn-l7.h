...
{
    gen_opt_add(&event_opts->event_opts[event_type], opt_name, opt_code, opt_type);
}
...
controller_event_opts_init(struct controller_event_options *opts)
{
    ...
    controller_event_opt_add(opts, OVN_EVENT_EMPTY_LB_BACKENDS, "load_balancer",
                             EMPTY_LB_LOAD_BALANCER, "str");
}
